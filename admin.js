...
let isFirstLoadFood = true;
let isFirstLoadService = true;

function playNotificationSound() {
    const sound = document.getElementById('order-sound');
    if (sound) {
        sound.currentTime = 0;
        sound.play().catch(e => console.log("Audio play interaction required:", e));
    }
}
...
database.ref('service_requests').on('value', (snapshot) => {
    ...
    if (data) {
        ...
        if (!isFirstLoadService) {
            playNotificationSound();
        }
    }
    isFirstLoadService = false;
